...
var BaseGhost = function () {
	...
	return function (_PIXI$mesh$Rope) {
		_inherits(_class, _PIXI$mesh$Rope);

		function _class(texture) {
			_classCallCheck(this, _class);
			...
			var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(_class).call(this, texture, points));

			_this.blendMode = PIXI.BLEND_MODES.ADD;
			_this.points = points;
			_this.moveSpeed = randfRange(0.05, 0.1);
	...
	return function (_PIXI$Container) {
		...
		function _class(config) {
			...
			function addRandomFrom(node) {
				...
				//(2)
				function addImage() {
					var xframe = randi(node.tiles_x - 1);
					var yframe = randi(node.tiles_y - 1);
					...
					if (!added) self.addChild(img);
				}

				//(1)
				if (texture.baseTexture.hasLoaded) addImage();else texture.baseTexture.once('loaded', addImage);
			}
			...
			function process(loader, resources) {
				...
				var topperSelect = randElem(["Horns", "Ears", "Hats"]);
				var mouthSelect = randElem(["Mouth", "Mustache"]);
				for (var i in res) {
					if (res[i].name.startsWith("BASE") || res[i].name.startsWith("Eyes") || res[i].name.startsWith("Brows") || res[i].name.startsWith("Emocii") || res[i].name.startsWith(topperSelect) || res[i].name.startsWith(mouthSelect) || res[i].name.startsWith("Paws") || res[i].name.startsWith("Glasses")) addRandomFrom(res[i]);
				}
			}
			return _this;
		}
...
(function () {
	function initTableStuff() {
...

		{
			...
			var bitmapText = new PIXI.extras.BitmapText(score, { font: "40px digital-regular", align: "left" });
			item.addChild(bitmapText);
			bitmapText.y -= 155;
			bitmapText.x -= 30;
		}
		...
		_createClass(_class, [{
			...
			value: function onDragMove(event) {
				...
				if (obj.dragging == 2) {
					...
					obj.position.x = Math.min(Math.max(obj.position.x, -gm.app.screen.width / 2), gm.app.screen.width / 2);

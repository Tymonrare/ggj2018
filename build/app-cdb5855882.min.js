	...
	return function (_TableItem) {
		...
		_createClass(_class, [{
			...
			value: function sendGhost() {
				...
				createjs.Tween.get(this.highlightRopesRay, { loop: false }).to({ alpha: 1 }, 50, createjs.Ease.quartInOut).to({ alpha: 0.7 }, 1000, createjs.Ease.quartIn); //just return for next animation

				//scale in another tween
				createjs.Tween.get(this.highlightRopesRay.scale, { loop: false }).to({ x: 7, y: 2.5 }, 200, createjs.Ease.quadOut).to({ x: 5, y: 1 }, 1000, createjs.Ease.quartIn); //just return for next animation
				...
				createjs.Tween.get(this.containsGhost, { loop: false, override: true }).to({ alpha: 1 }, 300, createjs.Ease.quartIn);

				this.containsGhost.moveToAbsolute(this.ropesContainer.worldTransform.tx + Math.cos(this.highlightRopesRay.rotation) * 1200, this.ropesContainer.worldTransform.ty + Math.sin(this.highlightRopesRay.rotation) * 1200);

				setTimeout(function () {
					_this2.allowTeleport = true; //do not accept new ghosts
					_this2.tranfering = false;

					_this2.containsGhost.linkPaper.removeFromGame();
					_this2.containsGhost.parent.removeChild(_this2.containsGhost);

					_this2.ropesContainer.scale.set(0);
					gm.play.soulsSent += 1;

					var g = _this2.containsGhost;
					var b = gm.play.boilers.souls[_this2.targetBoiler];
			...
			value: function handleGhosts(event) {
				if (!this.tranfering && gm.activeGhost && this.allowTeleport && this.crystal.getBounds().contains(event.data.global.x, event.data.global.y)) {
					...
					gm.activeGhost.moveToAbsolute(this.ropesContainer.worldTransform.tx, this.ropesContainer.worldTransform.ty);

					createjs.Tween.get(gm.activeGhost, { loop: false }).to({ alpha: 0 }, 1000, createjs.Ease.quartInOut);
		...
		_createClass(_class, [{
			...
			value: function onClick(event) {
				...
				if (this.state == 0) {
					...
					if (this.autoPutForward) this.putForward();
...
var BaseGhost = function () {
	...
	return function (_PIXI$Container) {
		...
		function _class(config) {
			...
			_this.colorName = randElem(Object.keys(gm.play.colors));
			_this.color = hashCode(gm.play.colors[_this.colorName]);
			_this.deathCause = randElem(gm.play.deathCauses);
			...
			var rowbody = gm.utils.loadCoa_tools_faces(config, { color: _this.color, seed: _this.randSeed }, function () {
				...
				_this.mouthType = rowbody.gm_mouthType;
				_this.topperType = rowbody.gm_topperType;
...
(function () {
	function initTableStuff() {
		{
			for (var i = 0; i < 10; i++) {
				...
				var item = new TableItem(new PIXI.Texture.fromImage("res/table_stuff/small/" + index + ".png"), { stands: stands, movable: true });
				item.position.set(randfRange(-gm.app.screen.width / 2, gm.app.screen.width / 2), -randf(gm.app.screen.height));
				if (stands) item.rotation = randfRange(-Math.PI / 10, Math.PI / 10);else item.image.rotation = Math.PI * 2 * Math.random();
				item.snap();
				gm.scene.projectionRoot.addChild(item);
			}
		}
		...
		for (var i in items) {
			var item = new TableItem(new PIXI.Texture.fromImage("res/table_stuff/small/" + items[i] + ".png"), { stands: false, movable: true });
			item.position.set(randfRange(-gm.app.screen.width / 2, gm.app.screen.width / 2), -randf(gm.app.screen.height));
			item.snap();
		...
		{
			var item = new TableItem(new PIXI.Texture.fromImage("res/table_stuff/spirit_cup.png"), { stands: true, movable: true });
...

		{
			(function () {
				...
				var scoreText = new PIXI.extras.BitmapText(score, { font: "70px digital-regular", align: "left" });
				item.addChild(scoreText);
				scoreText.y += 185;
				scoreText.x -= 150;

				var souls = ("" + gm.play.soulsSent).padStart(6, "0");
				var soulsText = new PIXI.extras.BitmapText(score, { font: "70px digital-regular", align: "left" });
	...
	function initCornerPapers() {
		...
		gm.scene.cornerPapers.updateRotations = function () {
			for (var i = 1; i < container.children.length; i++) {
				var rotation = Math.PI / 10 - Math.PI / 14 * i;
				container.children[i].cornerPos.rotation = rotation;
				if (container.children[i].state == 0) createjs.Tween.get(container.children[i], { loop: false }).to(container.children[i].cornerPos, 300 + i * 100, createjs.Ease.quartInOut);
			}
		};
			...
			setInterval(function () {
				...
				function processText(param) {
					for (var key in param) {
						text += "\t\t" + key + ": " + param[key] + "\n";
						if (param[key] > 1) combos["x" + param[key]] = (combos["x" + param[key]] || 0) + 1;else singles++;
					}
				}
	...
	function initTV() {
		...
		item.on('pointerdown', function () {
			createjs.Tween.get(item, { loop: false }).to({ y: -30 }, 300, createjs.Ease.elasticInOut);
			...
			if (tvinstanced) texture.baseTexture.source.volume = 0;
			...
			createjs.Tween.get(ctv, { loop: false }).to({ y: gm.app.screen.height * randfRange(0.5, 0.9) }, 700, createjs.Ease.bounceOut);
		});
	}
		...
		for (var i = 0; i < gm.play.boilers.count; i++) {
			...
			for (var bkey in b) {
				for (var key in b[bkey]) {
					if (b[bkey][key] > 1) combos["x" + b[bkey][key]] = (combos["x" + b[bkey][key]] || 0) + 1;else singles++;
				}
			}
		}
		for (var key in combos) {
	...
	gm.scene.init = function () {
		gm.play = {};
		gm.play.deathCauses = ["Ischaemic heart disease", "Stoke", "Lower respiratory", "Chronic obstructive", "Trachea cancer", "Bronchus cancer", "Lung cancer", "Diabetes mellitus", "Alzheimer disease", "Diarrhoeal disease", "Tuberculosis", "Road injury", "Brick injury", "Accident"];
		gm.play.colors = {};
...
gm.utils.loadCoa_tools_faces = function (config, params, callback) {
	...
	function process(loader, resources) {
		...
		var topperSelect = a_topperSelect[rand(a_topperSelect.length, params.seed)];
		var a_mouthSelect = ["mouth", "mustache"];
		var mouthSelect = a_mouthSelect[rand(a_mouthSelect.length, params.seed)];

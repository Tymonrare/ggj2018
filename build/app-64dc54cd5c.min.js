...
(function () {
	...
	function initMainPaper() {
		...
		function addPaper(index) {
			...
			paper.cornerPos.y -= 500;
			paper.cornerPos.x += 200;
			paper.cornerPos.rotation = Math.PI / 5 - Math.PI * (i / gm.play.boilers.count) / 7;
			paper.autoPutForward = false;

			var numberText = new PIXI.Text('Boiler ' + index, { fontFamily: 'Arial', fontSize: 16, fill: 0x111111 });
			numberText.y = 280;
			numberText.x = -200;
			paper.addChild(numberText);

			var basicText = new PIXI.Text('Basic text in pixi', { fontFamily: 'Arial', fontSize: 24, fill: 0x111111 });
			basicText.y = -280;
			basicText.x = -200;
			...
			setInterval(function () {
				...
				text += " DEATH CAUSES:\n";
				for (var key in b.deathCauses) {
					text += "\t\t" + key + ": " + b.deathCauses[key] + "\n";
				}text += " TOPPERS:\n";
				for (var key in b.toppers) {
					text += "\t\t" + key + ": " + b.toppers[key] + "\n";
				}text += " MOUTHS:\n";
				for (var key in b.mouths) {
					text += "\t\t" + key + ": " + b.mouths[key] + "\n";
				}text += " COLORS:\n";
				for (var key in b.colors) {
					text += "\t\t" + key + ": " + b.colors[key] + "\n";
				}basicText.text = text;
			}, 100);
		}

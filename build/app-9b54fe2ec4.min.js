	...
	return function (_TableItem) {
		...
		_createClass(_class, [{
			...
			value: function sendGhost() {
				...
				createjs.Tween.get(this.highlightRopesRay, { loop: false }).to({ alpha: 1 }, 50, createjs.Ease.quartInOut).wait(2000).to({ alpha: 0.7 }, 500, createjs.Ease.quartInOut); //just return for next animation

				//scale in another tween
				createjs.Tween.get(this.highlightRopesRay.scale, { loop: false }).to({ x: 7, y: 2 }, 100, createjs.Ease.quadOut).wait(1500).to({ x: 5, y: 1 }, 500, createjs.Ease.linear); //just return for next animation

				this.allowTeleport = true; //do not accept new ghosts

				createjs.Tween.get(this.getChildByName("ropes_pivot"), { loop: false }).to({ alpha: 0 }, 3000, createjs.Ease.quartInOut);
				this.glitchSrt = 0;
